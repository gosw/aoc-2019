...
namespace csharp.gosw.solutionDay2
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            int[] currentProgram = new int[173];

            for (int i = 0; i < 99; i++)
            {
                for (int j = 0; j < 99; j++)
                {
                    Array.Copy(codes, currentProgram, codes.Length);
                    using (var computer = new Computer().Instance(currentProgram))
                    {
                        if (computer.RunProgram(i, j) == 19690720)
                        {
                            Console.WriteLine("Output: " + ((100 * i) + j));
                            Console.ReadLine();
                            break;
                        }
                    }
